...
area20  = pd.to_numeric(df["Unnamed: 4"], errors="coerce")  # AREALANDSQMI


def scaling_analysis(place, pop, area, label="YEAR"):

    # Require UA > 50,000 population
    mask = (
        place.notna() &
        pop.notna() &
        area.notna() &
    ...
    print(f"\n===== {label} Scaling Results (population > 50,000) =====")
    print(f"n = {n}")
    ...
    plt.scatter(pop_f, area_f, s=12, alpha=0.6)
    xx = np.linspace(x.min(), x.max(), 300)
    yy = model.params[0] + model.params[1] * xx
    plt.plot(10**xx, 10**yy, "r-", lw=2)
    plt.xscale("log")
    plt.yscale("log")
    plt.xlabel("Population 2020 (>50k)")
    plt.ylabel("Area (sq miles)")
    #plt.title(f"Urban Areas 2020 (>50k): Area vs Population")
    plt.tight_layout()
    plt.savefig('scaling_2020_50K.pdf')
    plt.show()
    ...
    plt.ylabel("Density")
    #plt.title(f"Residual Distribution 2020 (>50k)")
    plt.legend()
    plt.tight_layout()
    plt.savefig('distribution_2020_50K.pdf')
    plt.show()
    ...
    plt.title("2020 QQ-Plot vs Normal")

    plt.subplot(1,2,2)
    stats.probplot(resid, dist=stats.t,
                   sparams=(t_df, t_loc, t_scale), plot=plt)
    plt.title("2020 QQ-Plot vs Student-t")

    plt.tight_layout()
    plt.savefig('QQ_2020_50K.pdf')
    plt.show()
...
# Run 2020 (>50k)
results_2020_gt50k = scaling_analysis(place20, pop20, area20, label="2020 (>50k)")
