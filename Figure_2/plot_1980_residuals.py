...
# LOAD 1980 DATA
# --------------------------------------------------------
df = pd.read_csv("1980-Table_1.csv")  # adjust path if needed

# Columns from inspection:
#   Unnamed: 3  -> Place
#   Unnamed: 4  -> Population (1980)
#   Unnamed: 5  -> Land area (sq miles)

place80 = df["Unnamed: 3"]
pop80   = pd.to_numeric(df["Unnamed: 4"], errors="coerce")
area80  = pd.to_numeric(df["Unnamed: 5"], errors="coerce")

mask = place80.notna() & (pop80 > 0) & (area80 > 0)
place_f = place80[mask]
pop_f   = pop80[mask]
area_f  = area80[mask]
...
# Confidence intervals (robust to ndarray/DataFrame)
ci = model.conf_int()
...
print("\n===== 1980 MOST NEGATIVE RESIDUALS =====")
print(lowest.to_string(index=False))

print("\n===== 1980 MOST POSITIVE RESIDUALS =====")
...
plt.xscale("log")
plt.yscale("log")
plt.xlabel("Population 1980")
plt.ylabel("Area (sq miles)")
#plt.title("Urban Areas 1980: Area vs Population (log-log)")
plt.tight_layout()
plt.savefig('scaling_1980.pdf')
plt.show()
...
plt.ylabel("Density")
#plt.title("1980 Residual Distribution")
plt.legend()
plt.tight_layout()
plt.savefig('distribution_1980.pdf')
plt.show()
...
plt.title("1980 QQ-Plot vs Normal")

plt.subplot(1,2,2)
stats.probplot(resid, dist=stats.t, sparams=(t_df, t_loc, t_scale), plot=plt)
plt.title("1980 QQ-Plot vs Student-t")

plt.tight_layout()
plt.savefig('QQ_1980.pdf')
plt.show()
