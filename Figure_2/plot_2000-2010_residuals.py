...
# =========================================================
#  Helper: scaling fit + residual analysis + plots
# =========================================================
...
def scaling_analysis(place, pop, area, label="YEAR"):
    ...
    # CIs robust to ndarray/DataFrame
    ci = model.conf_int()
    ...
    plt.tight_layout()
    plt.savefig('scaling_'+label+'.pdf')
    plt.show()

    # ---------- 2. Residual histogram with fits ----------
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('distribution_'+label+'.pdf')
    plt.show()
...
df = pd.read_csv("2000-2010-Table_1.csv")

# 2010 fields:
place10 = df["Unnamed: 2"]                 # UANAME
pop10   = pd.to_numeric(df["Unnamed: 5"], errors="coerce")  # UAPOP
area10  = pd.to_numeric(df["Unnamed: 7"], errors="coerce")  # UAAREALAND

# 2000 fields:
place00 = df["Unnamed: 4"]                 # UA00NAME
pop00   = pd.to_numeric(df["Unnamed: 6"], errors="coerce")  # UA00POP
area00  = pd.to_numeric(df["Unnamed: 8"], errors="coerce")  # UA00AREALAND
...
results_2010 = scaling_analysis(place10, pop10, area10, label="2010")
results_2000 = scaling_analysis(place00, pop00, area00, label="2000")
